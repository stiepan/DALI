...
def initialize_autograph(operator_overload=hooks.OperatorBase(),
  ...
  convert_modules : list, optional
      Provides a way to include extra modules that should be converted by the autograph. In particular,
      the modules specified here take the precedence over `do_not_convert_modules`, so
      that some submodules of the otherwise excluded modules can be converted.
