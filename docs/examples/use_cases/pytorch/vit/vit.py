...
def create_pipeline(rank=0, world_size=1, img_shape=img_shape, random_area=random_area,
                    random_aspect_ratio=random_aspect_ratio, num_classes=num_classes,
                    data_path=data_path, batch_size=batch_size, mixup_alpha=0.8, cutmix_alpha=1.,
    ...
    @pipeline_def(enable_conditionals=True, batch_size=batch_size, device_id=device_id,
                  num_threads=num_threads)
    def pipeline():
        image, label = fn.readers.file(name="Reader", shard_id=rank, num_shards=world_size,
...
pipe = create_pipeline(rank=rank, world_size=world_size)
pipe.build()
train_loader = DALIClassificationIterator(pipe, reader_name="Reader", fill_last_batch=False)

for i, (pipe_out, ) in enumerate(train_loader):
    assert isinstance(pipe_out["data"], torch.Tensor)
    assert isinstance(pipe_out["label"], torch.Tensor)
    print(pipe_out["data"].device, pipe_out["label"].device)
    break
