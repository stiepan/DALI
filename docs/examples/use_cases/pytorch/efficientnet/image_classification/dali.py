...
def training_pipe(data_dir, interpolation, image_size, output_layout, automatic_augmentation,
    ...
    if automatic_augmentation == "autoaugment":
        output = auto_augment.auto_augment_image_net(images, shape=[250, 250], interp_type=types.DALIInterpType.INTERP_NN)
    elif automatic_augmentation == "trivialaugment":
        output = trivial_augment.trivial_augment_wide(images, shape=[250, 250], interp_type=types.DALIInterpType.INTERP_NN)
    else:
        output = images
